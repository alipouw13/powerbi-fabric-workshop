...
service desk workload, and the asset estate.

## Synthetic data notice

Everything here is generated. Service names, CI names, incident numbers, sites,
teams, costs, and every metric are synthetic. **This is not Schwab data.** Do not
treat it as production, customer, or regulated data.
...
The generator is seeded, so the same arguments always produce the same data.
...
| **2. Wide Tableau extract** | `data/raw/tableau_extract/incident_report_extract.csv` | One flat 38-column denormalized table, the way a `.hyper` extract looks today. This is the "before". |
...
group. Row counts below are for the files currently in the repo (13 months of
history, 400 CIs). They scale with `--months` and `--ci-count`.

### Conformed dimensions

| File | Grain | Rows | Columns |
| --- | --- | ---: | --- |
| `dim_date.csv` | One row per day | 396 | `date_key`, `date`, `year`, `quarter`, `month`, `month_name`, `month_year`, `day_of_month`, `day_of_week`, `day_name`, `is_weekend`, `week_of_year`, `fiscal_year`, `fiscal_quarter` |
| `dim_service.csv` | One row per business service | 12 | `service_key`, `service_id`, `service_name`, `service_tier`, `business_unit` |
| `dim_configuration_item.csv` | One row per CI | 400 | `ci_key`, `ci_id`, `ci_name`, `ci_type`, `environment`, `criticality`, `service_key`, `location_key`, `support_group` |
...
| --- | --- | --- | ---: | --- |
| `fact_incident.csv` | 1, ITSM | One row per incident | 32,992 | `incident_key`, `incident_number`, `date_key`, `ci_key`, `service_key`, `team_key`, `location_key`, `severity_key`, `opened_at`, `resolved_at`, `incident_state`, `category`, `contact_type`, `time_to_resolve_minutes`, `reassignment_count`, `reopened_flag`, `sla_met_flag`, `major_incident_flag`, `incident_count` |
| `fact_capacity.csv` | 2, Capacity | One row per CI per month | 1,560 | `date_key`, `ci_key`, `service_key`, `location_key`, `cpu_utilization_pct`, `memory_utilization_pct`, `storage_allocated_gb`, `storage_used_gb`, `headroom_pct` |
| `fact_mainframe.csv` | 3, Mainframe | One row per LPAR per day | 7,524 | `date_key`, `ci_key`, `location_key`, `mips_consumed`, `mips_capacity`, `batch_jobs_completed`, `batch_jobs_failed`, `batch_window_minutes`, `transactions_processed` |
| `fact_service_desk.csv` | 4, Service Desk | One row per team per location per day | 2,376 | `date_key`, `team_key`, `location_key`, `tickets_received`, `tickets_resolved`, `first_contact_resolved`, `calls_abandoned`, `agents_scheduled`, `agents_available`, `avg_handle_time_minutes`, `avg_speed_to_answer_seconds` |
| `fact_asset.csv` | 5, Assets | One row per asset | 400 | `asset_key`, `asset_tag`, `ci_key`, `service_key`, `location_key`, `purchase_date`, `warranty_end_date`, `lifecycle_status`, `acquisition_cost_usd`, `annual_support_cost_usd`, `is_under_warranty`, `cmdb_complete_flag`, `asset_count` |
...
One table, **38 columns, 32,992 rows**, denormalized. Every service, CI, team,
site and severity attribute repeated on every incident row, plus two
...
and it is what the star-schema conversation is about. `site_name` appears 32,992
times here and 8 times in `dim_location`.
...
`data/raw/excel/capacity_YYYY_MM.csv`, one file per month (13 files as shipped).
...
**`capacity_2025_11.csv` is different on purpose.** Two changes:
